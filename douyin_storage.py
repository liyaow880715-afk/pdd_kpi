"""
抖音数据存储
- 店铺数据存放到 data/processed_douyin/{store_safe}/
- 每日保存：商品指标、订单
"""
...
DATA_DIR = Path("data/processed_douyin")
...
def list_douyin_stores() -> List[str]:
    _ensure_dir()
    stores = []
...
def list_douyin_records(store_name: Optional[str] = None) -> List[Dict[str, Any]]:
    _ensure_dir()
    records = []
