"""
天猫数据存储
- 店铺数据存放到 data/processed_tmall/{store_safe}/
- 每日保存：商品指标、订单
"""
...
DATA_DIR = Path("data/processed_tmall")
...
def list_tmall_stores() -> List[str]:
    _ensure_dir()
    stores = []
...
def list_tmall_records(store_name: Optional[str] = None) -> List[Dict[str, Any]]:
    _ensure_dir()
    records = []
