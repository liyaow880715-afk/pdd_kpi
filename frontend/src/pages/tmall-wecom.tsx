...
import {
  getTmallWecomConfig,
  updateTmallWecomConfig,
  sendTmallWecomReport,
} from "@/api/client"

export function TmallWecomPage() {
  const [config, setConfig] = useState<Record<string, any>>({})
  const [reportDate, setReportDate] = useState(new Date().toISOString().split("T")[0])
  const [message, setMessage] = useState("")

  useEffect(() => {
    getTmallWecomConfig().then(setConfig)
  }, [])
  ...
  const handleSave = async () => {
    try {
      await updateTmallWecomConfig(config)
      setMessage("配置已保存")
    } catch (err: any) {
  ...
  const handleSend = async () => {
    try {
      const res = await sendTmallWecomReport(reportDate, config)
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold">天猫企微</h2>
      {message && (
        <div
          className={`text-sm p-3 rounded-md ${
            message.includes("成功") || message.includes("保存") || message.includes("发送")
