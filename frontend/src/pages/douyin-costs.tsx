...
import {
  getDouyinCosts,
  saveDouyinCosts,
  refreshDouyinCostCodes,
  getDouyinUnmappedProducts,
  mapDouyinProductToMerchantCode,
  exportDouyinCosts,
  importDouyinCosts,
  type DouyinCost,
  type DouyinUnmappedRow,
} from "@/api/client"

export function DouyinCostsPage() {
  const [costs, setCosts] = useState<DouyinCost[]>([])
  const [unmapped, setUnmapped] = useState<DouyinUnmappedRow[]>([])
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const [costsData, unmappedData] = await Promise.all([getDouyinCosts(), getDouyinUnmappedProducts()])
      setCosts(costsData)
  ...
  const updateCost = (idx: number, field: keyof DouyinCost, value: any) => {
    const next = [...costs]
  ...
  const handleRefresh = async () => {
    try {
      const res = await refreshDouyinCostCodes()
      await fetchData()
  ...
  const handleExportPending = async () => {
    try {
      const blob = await exportDouyinCosts(true)
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = `抖音待维护商家编码_${new Date().toISOString().split("T")[0]}.csv`
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    try {
      const res = await importDouyinCosts(file)
      await fetchData()
  ...
  const mappingKey = (row: DouyinUnmappedRow) => `${row.product_id}::${row.style_id}`

  const handleMap = async (row: DouyinUnmappedRow) => {
    const key = mappingKey(row)
    const merchantCode = mappingCodes[key]
    if (!merchantCode) return
    try {
      await mapDouyinProductToMerchantCode(
        row.product_id,
        merchantCode,
  ...
  const renderCostTable = (rows: DouyinCost[], title: string, variant: "warning" | "success", icon: React.ReactNode) => {
    return (
