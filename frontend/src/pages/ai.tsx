...
import { getStores, getAiConfig, updateAiConfig, testAi, generateAiReport, type Store } from "@/api/client"
...
export function AiPage() {
  ...
  const handleReport = async () => {
    ...
    setReport("")
    try {
      const res = await generateAiReport(storeName, startDate, endDate, config)
  ...
  return (
    <div className="space-y-4">
      ...
      {message && (
        <div className={`text-sm p-3 rounded-md ${message.includes("成功") || message.includes("保存") || message.includes("连接") ? "bg-green-100 text-green-800" : "bg-destructive/10 text-destructive"}`}>
          {message}
        </div>
      )}
