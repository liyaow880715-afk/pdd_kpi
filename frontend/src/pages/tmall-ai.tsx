...
import {
  ...
  updateTmallAiConfig,
  testTmallAi,
  generateTmallAiReport,
  type Store,
} from "@/api/client"

export function TmallAiPage() {
  ...
  useEffect(() => {
    ...
    getTmallAiConfig().then(setConfig)
  }, [])
  ...
  const handleSave = async () => {
    try {
      await updateTmallAiConfig(config)
      setMessage("配置已保存")
    } catch (err: any) {
  ...
  const handleTest = async () => {
    try {
      const res = await testTmallAi(config)
  ...
  const handleReport = async () => {
    ...
    setReport("")
    try {
      const res = await generateTmallAiReport(storeName, startDate, endDate, config)
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold">天猫 AI</h2>
      {message && (
        <div
          className={`text-sm p-3 rounded-md ${
            message.includes("成功") || message.includes("保存") || message.includes("连接")
