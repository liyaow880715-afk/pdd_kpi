...
import { getStores, getDouyinAnalysis, getDouyinTrend, type Store } from "@/api/client"
...
export function DouyinMetricsPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const [analysis, trendData] = await Promise.all([
        getDouyinAnalysis(storeName, startDate, endDate),
        getDouyinTrend(storeName, startDate, endDate),
      ])
      setData(analysis)
      setTrend(trendData)
    } catch (err: any) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold">抖音指标</h2>
      <Card>
