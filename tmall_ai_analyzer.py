...
def test_ai_connection(config: Dict[str, Any]) -> str:
    ...
    return call_llm(
        ...
        system_prompt="你是一位资深的天猫电商投放分析师。",
...
TOP 计划/商品（按消耗排序）：
{chr(10).join(product_lines) if product_lines else '暂无计划/商品数据'}
"""
    return prompt
...
def generate_ai_report(
    ...
    try:
        content = call_llm(
            ...
            temperature=float(config.get("temperature", 1.0)),
            reasoning_effort=config.get("reasoning_effort", "low"),
            system_prompt="你是一位资深的天猫电商投放分析师，擅长用数据驱动决策。",
            timeout=int(config.get("timeout", 60)),
            max_completion_tokens=int(config.get("max_completion_tokens", 16384)),
        )
