...
def list_costs(_: dict = Depends(require_page("tmall_costs"))):
    return tmall_cost_manager.get_global_costs()
...
def export_costs(
    ...
    return tmall_cost_manager.export_global_costs_to_csv(pending_only)
...
def unmapped_products(
    ...
    return tmall_cost_manager.get_products_without_merchant_code(
...
def count_unmapped_products(
    ...
    start_s = start_date.strftime("%Y-%m-%d") if start_date else None
    end_s = end_date.strftime("%Y-%m-%d") if end_date else None
    df = tmall_cost_manager.get_products_without_merchant_code(
        store_names=store_names,
        start_date=start_s,
        end_date=end_s,
    )
...
def map_product_to_merchant_code(
    ...
    cfg = tmall_cost_manager.load_cost_config()
    cfg = tmall_cost_manager.save_global_product_mapping(
        cfg,
        req.product_id,
        req.merchant_code,
