...
def import_douyin_data(
    ...
    user: dict = Depends(require_page("douyin_import")),
):
...
def analysis(
    ...
    user: dict = Depends(require_page("douyin_metrics")),
):
    authorize_store(user, store_name)
    return douyin_services.load_douyin_analysis(store_name, start_date, end_date)
...
def trend(
    ...
    authorize_store(user, store_name)
    return douyin_services.load_douyin_trend(store_name, start_date, end_date)
...
def dashboard_summary(
    ...
    user: dict = Depends(require_page("douyin_overview")),
):
    ...
    if store_names:
        ...
        selected = allowed
    return douyin_services.get_douyin_dashboard_summary(start_date, end_date, store_names=selected)
...
def list_orders(
    ...
    user: dict = Depends(require_page("douyin_orders")),
):
    authorize_store(user, store_name)
    return douyin_services.get_douyin_orders(store_name, date)
...
def list_records(
    ...
    if user.get("role") == "master":
        return douyin_services.get_douyin_records(store_name)
    allowed = set(user.get("allowed_stores") or [])
    records = douyin_services.get_douyin_records(store_name)
...
def delete_record(
    ...
    authorize_store(user, store_name)
    return douyin_services.delete_douyin_record(store_name, date)
